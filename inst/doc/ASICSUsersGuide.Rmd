...
correction, normalisation and alignment on a reference spectrum 
(based on Vu et al. (2011)). For example, spectra are baseline corrected with:
...
```{r normalisation}
spectra_norm <- normaliseSpectra(spectra_base_cor, type.norm = "pqn")
```

The alignment algorithm is based on Vu et al. (2011). To find the reference 
spectrum, the LCSS similarity is used (Vlachos et al. (2002)). Then the 
...
only the function `ASICS`. This function takes approximately 2 minutes per 
...
*International Conference on* (pp. 673-684). IEEE.
