...
author: "Gaëlle Lefort, Rémi Servien, Patrick Tardivel and Nathalie Villa-Vialaneix"
date: "`r Sys.Date()`"
output:
...
unix-like OS using the BiocParallel package of Bioconductor. Parallel 
environment is switched on/off with the option `parallel` of the relevant 
functions.
...
used as references for quantification: only metabolites that are present in the library 
object can be quantified with ASICS.
...
correction, normalisation by the AUC and alignment on a reference spectrum (Vu et al., 2011). 
For example, here, spectra are baseline corrected:
...
Some analyses functions are available in ASICS package.

First, a design data frame is imported. In this data frame, the first column 
need to correspond to sample names of all spectra. 
```{r design}
design <- read.table(system.file("extdata", "design_diabete_example.txt", 
                                 package = "ASICSdata"), header = TRUE)
```

Then, a preprocessing is performed on relative quantification: metabolites with
more than 75% of null quantification are removed as well as two samples.
```{r analyses_obj}
...
It's also possible to find differences between two conditions with an OPLS-DA 
(Thevenot *et al*, 2015) or Kruskall-Wallis tests:
```{r oplsda}
...
the spectra bucketing:
```{r align_and_binning, cache=TRUE, results='hide'}
...
Finally, all analyses can be carried out on this object with the parameter `type.data` equal to `buckets`. 
For example, the OPLS-DA:
```{r oplsda_buckets}
