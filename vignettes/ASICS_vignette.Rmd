---
title: "ASICS package"
author: "Vignette Author"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{ASICS package}
  %\VignetteEncoding{UTF-8}
  %\VignetteEngine{knitr::rmarkdown}
editor_options: 
  chunk_output_type: console
---

This vignette provides an overview of the package ASICS. ASICS is a completely 
automated procedure to identify and quantify metabolites in $^1$H NMR spectra of 
biological mixtures. It will enable empowering NMR-based metabolomics by quickly 
and accurately helping experts to obtain metabolic profiles.

```{r ASICSload}
library(ASICS)
```

## Library of pure NMR metabolites spectra

A dataset with spectra of 175 pure metabolites is available in the package. It
is automatically loaded at package start and available metabolites are displayed
with:
```{r lib}
head(pure_library$name, n = 20)
```

## Identification and quantification of metabolites on one or more Bruker files with ASICS

The identification and the quantification are carried out using only one function \code{ASICS} directly 
from raw files (Bruker files):
```{r ASICS_onefile}
# part of the spectrum to exclude (water)
to_exclude <- matrix(c(4.5,5.1,5.5,6.5), ncol = 2, byrow = TRUE)
result <- ASICS(path = system.file("extdata", "example_spectra",
                                   "AG_faq_Beck01", package = "ASICS"),
                exclusion.areas = to_exclude)
print(result)
summary(result)
```

The quality of the results can be assessed by stacking the original and 
the recomposed spectrum on one plot. Pure metabolites spectrum can also be 
added for visual comparison.
```{r plot_spectrum, warning=FALSE, fig.width=10, fig.height=6}
plot(result, xmin = 1, xmax = 1.5, ymax = 10, add_metab = "Lactate")
```

Relative concentrations of identified metabolites are saved in:
```{r rel_conc}
result@present_metabolites
```

Usually, many spectra are available for one experience. With the 
\code{ASICS_multiFiles} function, quantification is launched on all spectra 
present in the specified folder (\code{name.dir}). The arguments of this function are the 
same than for \code{ASICS} function with the addition of \code{ncores} for  parallel computation (if wanted).
                                                                                                         
```{r ASICS_multifile, results = 'hide', eval = FALSE}
res_multi <- ASICS_multiFiles(name.dir = system.file("extdata",
                                                     "example_spectra",
                                                     package = "ASICS"),
                              exclusion.areas = to_exclude, ncores = 4)
```

For further analyses, it is possible to extract concentrations of all spectra in
a data-frame:
```{r extract_concentrations, eval = FALSE}
quantification <- extract_concentrations(res_multi)
summary(quantification)
```


## Reference 

TODO


## Session information

This vignette has been created with the following system configuration
```{r sysinfo}
sessionInfo()
```




